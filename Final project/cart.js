...
function CartDAO(database) {
    ...
    this.itemInCart = function(userId, itemId, callback) {
        ...
        collection.find(
            ...
            { "items.$": 1 }).toArray(function(err, doc) {
                console.log(doc)
                doc.length !== 0 ? callback(doc[0].items[0]) : callback(null);
            });
    ...
    this.updateQuantity = function(userId, itemId, quantity, callback) {
        ...
        collection.findOneAndUpdate(
            ...
            {
                projection: { userId: 1, items: 1 },
                returnOriginal: false,
                upsert: true
            }, function(err, doc) {
              console.log(doc);
                // var items = doc.value.items.filter(function(item) {
